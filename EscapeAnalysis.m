...
load('CSFcN_ablation_Escape.mat')
...
datasetPerBout3 = datasetPerBout([datasetPerBout(:).NumberOfOscillations]>1.5);
datasetPerBout1 = datasetPerBout3([datasetPerBout3(:).NStim]<11); %trial number, NStim<6 for the first five trials
...
for i = 1:length(datasetPerBout1)
            ...
            datasetPerBout1(i). Latency = ((diff_Angle(1)+ datasetPerBout1(i).BoutStart)-126)/635*1000;
            datasetPerBout1(i).Cbend = abs(datasetPerBout1(i).AmplitudeOfAllBends(1));
            datasetPerBout1(i).BendTime = (datasetPerBout1(i).Bend_Timing(1)-diff_Angle(1)+1)*1.58;
...
datasetPerBout1= datasetPerBout1([datasetPerBout1(:).New_BoutStart]>125); %response after stimulus
...
exlfilename = 'AllTrialsOutput_escape0403.xlsx';
